# app_tavily_fixed_v11_high_precision_eval.py
"""
EvoGen AI with Tavily integration (v11.0: 高精度評価プロンプト版)

v10.1 (app_tavily8.py) からの変更点:
- (評価精度向上) ユーザーの要望に基づき、評価エージェントのロジックを変更。
- `get_agent_personas_prompt` (修正): 
  - 評価エージェントが `criteria` (評価基準リスト) を生成するのを廃止。
  - 代わりに、その役割が厳密な評価を行うための「evaluation_guideline」(詳細な評価ガイドライン) 
    を生成するようにプロンプトを修正。
- `get_evaluation_prompt` (修正):
  - `criteria` をパースするロジックを削除。
  - AIが生成した `role` と `evaluation_guideline` に基づき、
    より精度の高い評価(総合スコア、強み、弱み)を要求するプロンプトを
    動的に構築するように全面的に書き換え。

v10.1 の特徴 (v10.0から変更):
- (機能) Tavilyでリサーチ後にLLMが生成した「補強された課題文」
  (`augmented_problem`) を、StreamlitのUI上に
  エキスパンダー（アコーディオン）で表示するように修正。
  
v10.0 の特徴 (v9.0から変更):
- (汎用性) `get_agent_personas_prompt` を修正し、
  AIが課題に応じて最適な「評価エージェNT」をゼロから設計するように指示。
- (検索強化) Tavilyの検索クエリ数を2から4に増加。
"""
...
class PromptManager:
    ...
    def get_agent_personas_prompt(self, problem_statement: str) -> str:
        ...
        - `evaluation_guideline`: (★修正点★) その役割が解決策を厳密に評価するために使用する、**具体的かつ詳細な評価指針（ガイドライン）**。このガイドラインには、何を最重要視し、どのような観点で優劣を判断すべきかを明確に記述してください。(従来の `instructions` と `criteria` を置き換えるものです)
    ...
    def get_evaluation_prompt(self, solution: Dict[str, str], problem_statement: str, context: Dict[str, Any]) -> str:
        ...
        精度の高い評価用プロンプトを動的に構築する。
        """
        
        evaluator_role = context.get('role', 'あなたは客観的で厳しい批評家です。')
        # ★修正点: 'instructions' や 'criteria' の代わりに 'evaluation_guideline' を使用
        evaluation_guideline = context.get('evaluation_guideline', '提示された解決案を、課題の要件に基づき厳密に評価してください。')
        ...
        # タスク
        あなたの「役割」と「最重要評価ガイドライン」に厳密に従い、上記の「解決案」を評価してください。
        ガイドラインに照らして、この解決策が課題をどれだけ効果的に解決できるか、または劣っているかを具体的に分析してください。
...
class EvoGenSolver:
    ...
    def _evaluate_solutions(self, solutions: List[Dict[str, str]], problem_statement: str, context: Dict) -> Generator[str | List[Dict], None, None]:
        # (v9.0のまま / v11.0 の高精度評価プロンプトでも集計ロジックは変更不要)
...
"--- (以下、元の課題文) ---\n" + problem_statement
    ...
    def solve(self, problem_statement: str, generations: int = 3) -> Generator[str | Dict, None, None]:
        ...
        # 拡張された問題文（または元の問題文）をUIに渡す
        yield {"augmented_problem": augmented_problem}

        # 拡張された問題文で EvoGen の本体 (v11.0 の高精度評価スウォームロジック) を実行
        yield from super().solve(augmented_problem, generations)


# ----------------------------
# 6) Streamlit UI (★v11.0: チーム表示部分のみ微修正★)
# ----------------------------
...
if st.button("解決策の生成を開始", type="primary"):
    if not gemini_key:
        ...
        with st.spinner("🌀 AIが思考中です..."):
            ...
            # --- Solverを実行し、結果をUIにストリーミング表示 (v10.1のまま) ---
            for result in solver.solve(problem_statement, generations=num_generations):
                if isinstance(result, str):
                    ...
                    # (★v11.0: 評価チーム表示を evaluation_guideline に対応)
                        ...
                        with st.expander("チームの詳細を表示"):
                            ...
                            if eva_list:
                                for i, eva in enumerate(eva_list):
                                    ...
                                    # ★修正点: criteria ではなく evaluation_guideline を表示
