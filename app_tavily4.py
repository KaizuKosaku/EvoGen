# app_tavily_fixed_v2.py
"""
EvoGen AI with Tavily integration (v2: 出力形式安定化版)

使い方:
  - 必要ライブラリ:
      pip install streamlit requests google-generativeai
  - 実行:
      streamlit run app_tavily_fixed_v2.py
"""
...
class PromptManager:
    ...
    def get_tavily_query_generation_prompt(self, problem_statement: str) -> str:
        ...
        以下の「課題」を分析し、**その解決策を考案するために**最も有用な情報を取得できる、具体的な**日本語の検索クエリ**を4つ生成してください。
        
        生成するクエリは、単なる課題の言い換えではなく、
        「最新の対策事例」「関連する技術の動向」「課題対象の詳細情報や最新データおよび制約条件」「対象の現状や事前調査」といった、解決策の質を高めるための**ファクトベースの情報**に焦点を当てたものにしてください。
